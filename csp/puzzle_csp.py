...
def binary_ne_grid(fpuzz_grid):
    ...
    # constraints required contains the following: first should ensure each row and each column
...
def nary_ad_grid(fpuzz_grid):
    ...
    # first find all diagonal indices, and for each of these, extract the row and column associated
    # and create constraint on it
    for i in range(board_length):
...
def caged_csp_model(fpuzz_grid):
    ...
    # at the same time, given each separate constraints, should consider the values possible of making the constraints
    # satisfied;
    # if all of these are done, we might no longer need the constraints about creating board.
    # generate all possible choices of numbers in the area, and see whether all these combinations
